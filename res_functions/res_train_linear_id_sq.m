function [Wout, S, S_store] = res_train_linear_id_sq(A,Win,v,vT,trans,q,eps0,leak,bias,reg)
...
%   S: feature vector of last state, [R(:,tt);R(:,tt).^2;v(:,tt-1);1]
...
tt = size(v,2);
vR = zeros(input_size-1,2*num_nodes+input_size+1);
RR = zeros(2*num_nodes+input_size+1);
Savg = zeros(2*num_nodes+input_size+1,tt);

for i = 1:q
    R = zeros(num_nodes,tt);
    S = zeros(2*num_nodes+input_size+1,tt);
    
    for j = 1:tt-1
        noise = eps0.*(1 - 2*rand(input_size-1,1));
        u = v(:,j) + [noise;0];
        R(:,j+1) = res_advance(A,Win,R(:,j),u,leak,bias);
        S(:,j+1) = [R(:,j+1);R(:,j+1).^2;v(:,j);1];
    end
